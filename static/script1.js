...
function handleScroll() {
  ...
  const totalCarouselHeight = windowHeight * 3;

  let percent = scrollY / totalCarouselHeight;
  if (percent > 1) percent = 1;

  const totalSlides = 3;
  const snapStep = 1 / totalSlides;

  const snapIndex = Math.round(percent / snapStep);
  const snappedPercent = snapIndex * snapStep;

  const rotateY = snappedPercent * 360;
  carouselList.style.transform = `rotateY(${rotateY}deg)`;
}
...
function swirl() {
  currentAngle -= 360;
  carouselList.style.transition = 'transform 1s ease';
  carouselList.style.transform = `rotateY(${currentAngle}deg)`;
}
